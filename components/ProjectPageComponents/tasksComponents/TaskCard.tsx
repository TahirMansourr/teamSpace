...
const TaskCard = ({task } : {task : TaskDto}) => {
  ...
  return (
    <section className='flex flex-col w-[95%] shadow-md m-2 rounded-md p-2 border'>
       <h5 className=' font-bold underline-offset-2 underline'>{task.name}</h5>
       <p className='my-1'>
       <Badge
          radius='md'
          size="sm"
        ...
        <CreateOrUpdateTaskModal 
          ...
          />
       <h5 className=' text-sm font-bold my-2'>Due on: {date.toLocaleString()}</h5>
       <Spoiler maxHeight={40} showLabel="..." hideLabel="Hide">
        ...
        <div className="flex flex-col mt-2">
            ...
            <FiEdit className=' ml-auto hover:cursor-pointer hover:scale-110' color='blue' onClick={open}/>
        </div>
