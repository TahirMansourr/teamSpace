...
const FeaturesIssuesTab = ({featureId} : {featureId : string}) => {
  ...
  return (
    <section className='flex flex-col w-full'>
      <MdOutlineAddCard  onClick={open} className=' ml-auto hover:cursor-pointer hover:scale-105 shadow-sm' size={30} color='blue' />
      <CreateOrUpdateIssuesModal modalOpened = {modalOpened} closeModal={closeModal} FeatureId={featureId}/>
      <div className="grid grid-cols-3 mt-4">
        {
            allFeatureIssues.length > 0 ? allFeatureIssues.map((issue : IssueDto) => (
