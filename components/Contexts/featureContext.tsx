...
interface FeatureContextDto {
  ...
  submitCreateFeatureForm: (values: {
    name: string;
    description: string;
  }) => void;
}

const FeautreContext = createContext<FeatureContextDto>(
  {} as FeatureContextDto
);
...
const FeatureProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  async function submitCreateFeatureForm(values: {
    name: string;
    description: string;
  }) {
  ...
  return (
    <FeautreContext.Provider value={value}>{children}</FeautreContext.Provider>
  );
};
