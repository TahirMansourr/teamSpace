...
const WorkSpaceContext = createContext<WorkSpaceContextDto>(
  {} as WorkSpaceContextDto
);
...
const WorkSpaceProvider = ({
  ...
  const [notesComponentExpandState, setNotesComponentExpandState] =
    useState<boolean>(false);
  const [tasksComponentExpandState, setTasksComponentExpandState] =
    useState<boolean>(false);
  const [issuesComponentExpandState, setIssuesComponentExpandState] =
    useState<boolean>(false);
  const [chatComponentExpandState, setChatComponentExpandState] =
    useState<boolean>(false);
  const [project, setProject] = useState<projectInfoResponse>(
    projectInfo ? projectInfo : ({} as projectInfoResponse)
  );
